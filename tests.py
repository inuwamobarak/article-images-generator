...
class StableDiffusionLitAPI(ls.LitAPI):
    def setup(self, device):
        ...
        self.model = StableDiffusionPipeline.from_pretrained(
            "stabilityai/stable-diffusion-2-1", torch_dtype=torch.float16, use_auth_token=True).to(device)
        self.model = DiffusionPipeline.from_pretrained("CompVis/stable-diffusion-v1-4")
